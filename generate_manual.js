...
const hmiSection = [ // eslint-disable-line no-unused-vars
  ...
  bulletPara("SPEED (Hz) and ACCEL (ms): Configurable for each zone independently. These values apply to normal production feeding — not to the pre-alignment cycle.", "bullets8"),
  ...
  bulletPara("Belt Selection – Either the outer belt, the inner belt, or both can be configured to run in reverse during the pre-alignment cycle.", "bullets8"),
  bulletPara("Number of Cycles – The pre-alignment sequence can be repeated multiple times per portion for better product positioning. Each additional cycle adds time to the bagging sequence and reduces overall machine output — use the minimum cycles needed for consistent alignment.", "bullets8"),
  bulletPara("Cycle Timers – When using more than one cycle, two independent timers must be configured: one for how long each belt runs in reverse per cycle, and one for the interval between cycles.", "bullets8"),
  bulletPara("Speed (Hz) and Accel (ms) – These settings control belt speed and ramp-up time during the pre-alignment cycle only. They are independent from the normal production belt speeds configured on the V-Drive Parameters page and do not affect feeding speed during the bagging cycle.", "bullets8"),
  spacer(40),
